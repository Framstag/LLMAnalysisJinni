...
- **WHEN** a module has >50% public methods across all classes (excluding test code)
- **THEN** the evaluation SHOULD flag this as a potential encapsulation concern

#### Scenario: Anemic domain model detected
- **WHEN** a module has >80% public methods with no private or protected methods on domain classes (excluding getters/setters, builders)
- **THEN** the evaluation SHOULD flag this as a possible anemic domain model

#### Scenario: Static method overuse
- **WHEN** a module has >40% static methods in production code
