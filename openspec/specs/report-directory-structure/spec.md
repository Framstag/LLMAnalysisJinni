...
CSV report files are organized into subdirectories named after the report type. Each per-module report uses the module name as the filename body to prevent overwriting across loop iterations. SBOM reports (no module context) use the report name as fallback filename.
...
All CSV report calls in `JavaTool` SHALL construct the filename as `<reportType>/<sanitizedModuleName>.csv`. SBOM calls in `SBOMTool` SHALL use `<reportType>/<reportType>.csv` since they have no module context.
