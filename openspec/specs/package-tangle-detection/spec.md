# Package Tangle Detection

## Purpose

Provides tools and evaluation tasks for detecting circular dependencies between packages within a build module, using the same import-based dependency graph as class-level cycle detection but aggregated at the package level. Enables identification of package-level architecture cycles that indicate deeper modularization issues than individual class-level cycles.
...
- **THEN** the evaluation SHOULD flag this as a major architecture erosion concern requiring refactoring