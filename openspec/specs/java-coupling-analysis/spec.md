...
Provides tools and evaluation tasks for analyzing per-class efferent coupling (Ce) and inter-module dependency patterns across production, test, and generated code in a module. Enables detection of hub classes (high fan-out), tightly-coupled modules, and dependency concentration on external frameworks.
...
- **THEN** the system returns 3 `Distribution` objects for class-level coupling (prod/test/gen) plus `Distribution` objects for module-level dependency counts
...
- **WHEN** more than 50 percent of a module's imports resolve to a single external module
